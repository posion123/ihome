    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        ...
        success:function(data){
            ...
            for(i=0; i<data.data.length;i++){
                node = '<li area-id="'+ data.data[i].id +'">' + data.data[i].name + '</li>'
                $('.filter-area').append(node)

            };

        }
    });
    var my_url = location.search
    $.ajax({
        url:'/user/my_search/' + my_url,
        dataType:'json',
        type:'GET',
        ...
        success:function(data){
            ...
            for (i=0;i<data.data.length;i++){
            node = ''
            node += '<li class="house-item">'
            node += '<a href="'+ '/house/detail/?id=' + data.house_sort[i].id + '">'

            node += '<img src="' + '/static/media/' + data.house_sort[i].image +'"></a>'
            node += '<div class="house-desc">'

            node += '<div class="house-price">' + '￥' + '<span>' + data.house_sort[i].price + '</span>/晚</div>'
            node += '<div class="house-intro">'
            node += '<span class="house-title">' + data.house_sort[i].title + '</span>'
            node += '</div></div></li>'
